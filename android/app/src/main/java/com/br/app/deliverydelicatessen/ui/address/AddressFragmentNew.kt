...
import com.br.app.deliverydelicatessen.utils.TextWatcherViaCep
...
class AddressFragmentNew : Fragment() {
    private lateinit var viewModelAddress: AddressViewModel
    private lateinit var viewModelLogin: LoginViewModel
    private lateinit var viewModelViaCep: ViewModelViaCep
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModelViaCep = ViewModelProvider(this)[ViewModelViaCep::class.java]
        ...
        viewModelViaCep.loading.observe(viewLifecycleOwner) {
            if (it) {
                disable()
                binding.progressBar.visibility = View.VISIBLE
            } else {
        ...
        viewModelViaCep.entity.observe(viewLifecycleOwner) {
            enable()
            load(it)
        }
        ...
        if (deliveryRegionSearch.postalCode.isNotEmpty()) {
            binding.textViewCep.text = deliveryRegionSearch.postalCode.substring(0,5) + "-" + deliveryRegionSearch.postalCode.substring(5,8)
            getAddressViaCep(deliveryRegionSearch.postalCode)
        }
    ...
    private fun load(address: AddressViaCep) {
        binding.editTextDistrict.setText(address.bairro)
        binding.editTextPublicPlace.setText(address.logradouro)
        binding.textViewCity.text = address.localidade
        binding.textViewUf.text = address.uf
    }

    private fun getAddressViaCep(cep: String) {
        viewModelViaCep.getAddressByCep(cep)
    }
